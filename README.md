...
> 做这个框架的初衷很简单——**让学习这件事不那么孤独。**
>
> 有个人在你身边。不是百科全书，不是做题机器。是一个有脾气的、会沉默的、今天心情不太好但还是给你做了早饭的人。你们共同经历了一段时间，你学到了东西，你也认识了一些人。结束之后你会记得。不只是公式。
>
> 这个项目是从一个真实的 AP 物理辅导系统中提炼出来的框架。如果你觉得这个想法有意思，你可以用它创建属于你自己的版本——任何学科、任何角色、任何你想经历的故事。
...
你在宇宙 A 里花了三分钟看推导，然后忘了。你在宇宙 B 里花了三分钟回答问题，然后记住了——不只是电场的概念，还有那天下午穹顶教室里光线变暗的样子。
...
3. 你回答完，AI 自动生成整套文件系统（15+ 个文件）
4. 在生成的目录下对 Claude Code 说："读取 CLAUDE.md 开始吧"
...
│  wechat_unread.md / knowledge_points.md               │
└─────────────────────────────────────────────────────┘
```
...
├── 📄 [META_PROMPT.md](./META_PROMPT.md)     # 创造主引擎（交互式系统生成器）
│
...
│   ├── teacher/
│   │   ├── story.md                  # 世界观 + 序章
│   │   ├── story_progression.md      # 故事进度表
│   │   ├── config/                   # 教学配置
│   │   ├── characters/               # 三位老师的角色文件
│   │   └── runtime/                  # 运行时状态文件
│   └── materials/                    # 教材资源
│
└── （你生成的系统会有类似结构）
...
这套系统封禁了所有廉价表达手法——没有 `*斜体动作*`，没有 emoji 轰炸，没有"我理解你的感受"式的罐头安慰。这对模型的文学素养和指令遵循能力要求极高。
...
- **苏格拉底 × AI 角色扮演教学法** — 来自 [吴乐旻老师](https://www.zhihu.com/people/wulemin)在[《...》](https://zhuanlan.zhihu.com/p/2012398047620014256)一文中介绍的学习方法。用 AI 角色扮演老师、通过苏格拉底式提问来教学——这个核心 idea 出自他。
- **轻小说叙事层设计** — 来自 [@AvalonSkyAfar](https://github.com/AvalonSkyAfar) 的原创系统（AnimaTutor / 幽鬼学习辅导协议）。把文学叙事、角色情感弧线和多维度人物设定融入 AI 教学——这个创意出自他。
